...
```{r}
# 1. Get predicted probabilities from part A
dat$prob_le6  <- predict(mA, type = "response")

# 2. Predicted Poisson mean for days with Y>6
dat_gt6       <- dat
dat_gt6$pred_gt6 <- predict(mB, type = "response")

# 3. Empirical mean among ≤6-days (your data codes them as "6")
mean_le6 <- 6  # or mean(dat$er_visits[dat$er_visits <= 6])

# 4. Combined expected ER visits for each observation
dat$E_Y <- dat$prob_le6 * mean_le6 +
           (1 - dat$prob_le6) * dat_gt6$pred_gt6

# 5. Average predicted visits by effect level
combined_effect <- dat[, .(expected_visits = mean(E_Y)), by = effect]
combined_effect
```
...
res <- two_part_gcomp_bootstrap(
  df        = dat_model,
  n_boot    = 100,          # increase to 500–1000 for publication
  cluster   = "zip",
  seed      = 123
)
