...
# Minimum absolute difference in days from the original date
min_diff_days <- 16L
...
# Optional: set a seed for reproducibility of the randomization
set.seed(123)

# Apply row-wise so geometry is preserved and function is evaluated per feature
Swaths.sf <- Swaths.sf %>%
  rowwise() %>%
  mutate(Date0_random = sample_hurr_date(as.Date(Date0), min_diff_days)) %>%
  ungroup()
```
...
TIC.sf <- Swaths.sf |>
    ...
    mutate(new_features = list(tidyr::tibble(
           Date = Date0_random + (-n_new:n_new) * deltaT))) |>
    unnest(new_features) |>
    ungroup() |>
